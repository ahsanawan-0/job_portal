...
export class CreateTestFormComponent implements OnInit {
  ...
  selectQuestion(question: any): void {
    const isAlreadySelected = this.questions.controls.some(
    ...
    if (!isAlreadySelected) {
      ...
      this.notification.showError('Question already selected!');
    }
  }
  ...
  unselectQuestion(index: number): void {
    ...
    this.notification.showError('Question has been unselected.');
  }
  ...
  createTest(): void {
    ...
    this.testService
      .createTest({ title, questions, duration: this.selectedDuration })
      .subscribe({
        next: () => {
          this.notification.showSuccess('Custom test created successfully!');
        },
        error: (error) => {
          this.errorMessage =
